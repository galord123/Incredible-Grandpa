use crate::{
    constants::{self, Access},
    evaluation, search,
    utils::get_piece_type,
};
use chess::{self, BitBoard, Board, CacheTable, ChessMove, Color, Piece, Square};
...
pub struct SearchInfo {
    ...
    pub transpositions_recorded: u64,
...
pub fn search_depth(
    board: &Board,
    depth: u32,
    sorted_moves: &Option<Vec<(ChessMove, i32)>>,
    max_time: u128,
    best_previous: (Option<ChessMove>, i32),
) -> (
    ...
    let mut cache_table = CacheTable::new(
        65536,
        Entry {
            depth: 0,
    ...
    let mut pawn_table = CacheTable::new(65536, 0);
    ...
    for chess_move in moves {
        ...
        if !checked_previous_best_move
            && chess_move == best_previous_move.unwrap_or(ChessMove::default())
        {
        ...
        total_transpositions_recorded += search_info.transpositions_recorded;
        total_transpositions_used += search_info.transpositions_used;
            ...
            if best_score >= best_previous_score + 100 {
                return (
                    ...
                    SearchInfo {
                        ...
                        transpositions_recorded: total_transpositions_recorded,
                        transpositions_used: total_transpositions_used,
                        pawn_hash_table_recorded: total_pawn_hash_table_recorded,
                        pawn_hash_table_used: total_pawn_hash_table_used,
                    },
                    true,
                );
            }
...
fn is_interesting(board: &Board, chess_move: ChessMove) -> bool {
    ...
    let black_pawn_push =
        pawn_push && board.color_on(source) == Some(chess::Color::Black) && dest <= Square::H3;
    let white_pawn_push =
        pawn_push && board.color_on(source) == Some(chess::Color::White) && dest >= Square::A6;
    let is_castle = board.piece_on(source) == Some(Piece::King)
        && (dest == Square::G1 || dest == Square::C1 || dest == Square::G8 || dest == Square::C8);
    let board_after_move = board.make_move_new(chess_move);
    let interesting = board.checkers().popcnt() > 0
        || move_is_capture
        || chess_move.get_promotion().is_some()
        || board_after_move.checkers().popcnt() > 0
        || black_pawn_push
        || white_pawn_push
        || is_castle;
    interesting
...
pub fn pv_search(
    ...
    beta: i32,
    depth: u32,
    cache: &mut chess::CacheTable<Entry>,
    info: &mut SearchInfo,
    pawn_table: &mut chess::CacheTable<i32>,
    pvline: &mut Vec<ChessMove>,
) -> i32 {
    let mut line: Vec<ChessMove> = Vec::new();
    ...
    for chess_move in moves {
        ...
        if first_search_pv {
            ...
            if !fprune || is_check(board, &chess_move) || fmax + gain(&board, &chess_move) > alpha {
                score = -zero_window_search(
                    &passed_board,
                    -alpha,
                    depth - 1 - moves_to_reduce,
                    info,
                    cache,
                    pawn_table,
                );
        ...
        if score >= beta {
            ...
            info.transpositions_recorded += 1;
...
fn zero_window_search(
    ...
    let using_cache = false;
    if depth <= 0 {
        return quiesce(board, beta - 1, beta, 6, info, pawn_table);
    }
    ...
    for chess_move in moves {
        let passed_board = board.make_move_new(chess_move);
        let score =
            -zero_window_search(&passed_board, 1 - _beta, depth - 1, info, cache, pawn_table);
...
pub fn pawn_table_lookup(
    ...
    // get endgame score
    let pieces_total_count = board.pieces(chess::Piece::Bishop).popcnt()
        + board.pieces(chess::Piece::Knight).popcnt()
        + board.pieces(chess::Piece::Rook).popcnt();
    ...
    match pawn_table.get(key) {
        None => {
            let structure_score =
                evaluation::evaluate_pawn_structure(black_pawns, white_pawns, endgame);
            // save to pawn table
